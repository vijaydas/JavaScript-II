...
// Problem 1: All donors over $150

let bigDonors = [];
bigDonors = runners.filter(runner => runner.donation > '150');
console.log(bigDonors);

// Problem 2: Need a quick runner bio 

let quickBio = [];
runners.forEach(function(runner) {
    quickBio.push(`${runner.first_name} ${runner.last_name} - ${runner.company_name}`);
})
console.log(quickBio);


// Problem 3: Need company names in a list in in ALL CAPS

let companyCaps = [];
companyCaps = runners.map(runner => runner.company_name.toUpperCase());
